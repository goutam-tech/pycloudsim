"""
Quantum Walk Based Scheduling Experiment.

Run with:
    python experiments/scheduling/run_qw.py
"""
...
from algorithms.scheduling.qw import QuantumWalkScheduler
...
csv_path = os.getenv("CSV_PATH_QW")
json_path = os.getenv("JSON_PATH_QW")
plots_path = os.getenv("PLOTS_PATH_QW")

def run() -> None:
    """Execute the Quantum Walk-based scheduling experiment."""
    print_banner("PyCloudSim — Quantum Walk Experiment")
    console.print()
    ...
    with make_progress() as progress:
        ...
        scheduler = QuantumWalkScheduler(
            alpha=0.5,
            beta=0.5,
            reps=1,
            batch_size=2,
    ...
    for vm in vms:
        ...
        metric(
            f"VM {vm.vm_id}: avg CPU utilisation = {util*100:.1f}%  |  cloudlets = {len(vm.cloudlets)}"
        )
