...
from algorithms.scheduling.qaoa.utils import get_qubit_index
...
def build_hamiltonian(
    ...
    # Ensure each cloudlet is assigned to exactly one VM
    # ------------------------------------------------------------------
    for i in range(n):
        for j in range(m):
    ...
    # ------------------------------------------------------------------
    # 2. Makespan and Load Balance (VM Completion Times)
    # Makespan proxy: Sum(T_j^2)
    # Load balance proxy: Sum((T_j - T_avg)^2)
    # Combined time term: (1.0 + beta) * Sum(T_j^2) - (beta / m) * (Sum(T_j))^2
    # ------------------------------------------------------------------
    preloads = [0.0] * m
    ...
    for j, vm in enumerate(vms):
        ...
        # Calculate preload (execution times of cloudlets already assigned)
        preload_len = sum(
    ...
    for j, vm in enumerate(vms):
        ...
        for i in range(n):
            ...
            # Quadratic contribution (same VM, different cloudlets)
    ...
    for i in range(n):
        ...
        for j in range(m):
            ...
            # Quadratic contribution (all pairs of variables)
            for k in range(n):
                cl_k = cloudlets[k]
                l_k = cl_k.length
                for l in range(m):
    ...
    # H_wait = alpha * Sum_j Sum_{i < k} x_{i,j} x_{k,j} * (L_i + L_k) / (2 * P_j)
    # ------------------------------------------------------------------
    for j, vm in enumerate(vms):
        p_j = vm.total_mips if vm.total_mips > 0.0 else 1.0
        for i in range(n):
    ...
    # Checks remaining RAM, Storage, and Bandwidth of VMs
    # ------------------------------------------------------------------
    for j, vm in enumerate(vms):
        # Calculate used resources (excluding current batch)
        used_ram = sum(
        ...
        for i in range(n):
            ...
            # Linear penalty if a single cloudlet exceeds VM capacity
            if (
                ram_req > avail_ram
            ...
            # Quadratic penalty for co-allocation of multiple cloudlets
            for k in range(i + 1, n):
                cl_k = cloudlets[k]
    ...
    # ------------------------------------------------------------------
    # Convert QUBO to Ising Hamiltonian (x_p = (I - Z_p) / 2)
    # ------------------------------------------------------------------
    offset = 0.0
    z_coeffs = [0.0] * q
    ...
    # Build Qiskit SparsePauliOp list
    pauli_list = []
    ...
    # If pauli_list is empty, return a dummy identity term
    if not pauli_list:
