"""
Bitstring Decoder and Constraint Repair for QAOA Scheduler.

Decodes measurement bitstrings from the quantum circuit into cloudlet-to-VM
assignments and applies conflict resolution heuristics to handle constraint violations.
"""
...
def decode_bitstring(
    ...
    for i in range(num_cloudlets):
        for j in range(num_vms):
            ...
            # If the bit is '1', the cloudlet is assigned to this VM
...
def repair_assignment(
    ...
    # Initialize current VM loads based on already scheduled workloads
    ...
    for i, cl in enumerate(cloudlets):
        ...
        # Validate candidate VMs against resource capacity constraints
        valid_candidates = []
        ...
        # Select the target VM index
        if valid_candidates:
            # Pick the valid VM with the lowest current workload
            target_vm_idx = min(valid_candidates, key=lambda j: vm_loads[j])
        else:
            # Fall back to any VM with the lowest load if capacities are all tight
            target_vm_idx = min(range(len(vms)), key=lambda j: vm_loads[j])

        # Record assignment and update simulated workload
