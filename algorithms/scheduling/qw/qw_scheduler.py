"""
Quantum Walk Based Cloudlet Scheduler for PyCloudSim.

Implements a cloudlet scheduler that maps cloudlets to VMs using a
discrete-time Quantum Walk on the hypercube graph, via Qiskit.
"""
...
from algorithms.scheduling.qw.hamiltonian import build_hamiltonian
from algorithms.scheduling.qw.decoder import decode_bitstring, repair_assignment
from algorithms.scheduling.qw.optimizer import get_optimizer
...
class QuantumWalkScheduler(CloudletScheduler):
    """
    Quantum Walk-based cloudlet-to-VM scheduler.

    Constructs a cost Hamiltonian representing makespan, waiting time,
    load balance, and capacity constraints, then uses a discrete-time
    Quantum Walk on the hypercube graph to find the optimal scheduling
    assignment.
    """
    ...
    def submit_cloudlets(self, cloudlets: List["Cloudlet"], vms: List["VM"]) -> None:
        """
        Optimize cloudlet assignment to VMs using Quantum Walk.
        ...
        """
        if not vms:
            raise ValueError("QuantumWalkScheduler requires at least one VM.")
        ...
        for start_idx in range(0, len(cloudlets), self.batch_size):
            ...
            # Build Quantum Walk ansatz without measurement gates for VQE
            from qiskit_algorithms import SamplingVQE
            from algorithms.scheduling.qw.circuit import build_qw_circuit

            num_qubits = len(batch) * len(vms)
            ansatz = build_qw_circuit(
            ...
            try:
                ...
                logger.error(
                    "Quantum Walk VQE optimization failed. Falling back to default: %s", exc
                )
